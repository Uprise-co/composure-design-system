!function (c, M) {
  ...
  function o(c) {
    ...
    for (45 === c.charCodeAt(0) && (n = -(M = 1)); M < A.length; M++) p = 60 * p + r(A.charCodeAt(M));

    for (M = 0; M < o.length; M++) b /= 60, p += r(o.charCodeAt(M)) * b;
  ...
  function O(c, M) {
    for (var a = [], A = 0; A < M.length; A++) a[A] = c[M[A]];
  ...
  function d(c) {
    var M = c.split("|"),
        ...
        A = M[3].split(""),
        o = M[4].split(" ");
    return q(a), q(A), q(o), function (c, M) {
  ...
  function t(c) {
    var M = c.toTimeString(),
        a = M.match(/\([a-z ]+\)/i);
    "GMT" === (a = a && a[0] ? (a = a[0].match(/[A-Z]/g)) ? a.join("") : void 0 : (a = M.match(/[A-Z]{3,5}/g)) ? a[0] : void 0) && (a = void 0), this.at = +c, this.abbr = a, this.offset = c.getTimezoneOffset();
  }
  ...
  function L() {
    for (var c, M, a = new Date().getFullYear() - 2, A = new t(new Date(a, 0, 1)), o = [A], b = 1; b < 48; b++) (M = new t(new Date(a, b, 1))).offset !== A.offset && (c = function (c, M) {
      for (var a, A; A = 6e4 * ((M.at - c.at) / 12e4 | 0);) (a = new t(new Date(c.at + A))).offset === c.offset ? c = a : M = a;

      return c;
    }(A, M), o.push(c), o.push(new t(new Date(c.at + 6e4)))), A = M;

    for (b = 0; b < 4; b++) o.push(new t(new Date(a + b, 0, 1))), o.push(new t(new Date(a + b, 6, 1)));
  ...
  function W() {
    ...
    for (var a, A, o = L(), b = o.length, p = function (c) {
      for (var M, a, A = c.length, o = {}, b = [], p = 0; p < A; p++) for (M in a = e[c[p].offset] || {}) a.hasOwnProperty(M) && (o[M] = !0);

      for (p in o) o.hasOwnProperty(p) && b.push(z[p]);

      return b;
    }(o), n = [], i = 0; i < p.length; i++) {
      for (a = new l(s(p[i])), A = 0; A < b; A++) a.scoreOffsetAt(o[A]);

      n.push(a);
    }

    return n.sort(u), 0 < n.length ? n[0].zone.name : void 0;
  }
  ...
  function X(c) {
    ...
    for ("string" == typeof c && (c = [c]), M = 0; M < c.length; M++) o = B(a = (A = c[M].split("|"))[0]), p[o] = c[M], z[o] = a, function (c, M) {
      var a, A;

      for (q(M), a = 0; a < M.length; a++) A = M[a], e[A] = e[A] || {}, e[A][c] = !0;
    }(o, A[2].split(" "));
  }
  ...
  function s(c, M) {
    ...
    var a,
        A = p[c];
    return A instanceof f ? A : "string" == typeof A ? (A = new f(A), p[c] = A) : n[c] && M !== s && (a = s(n[c], s)) ? ((A = p[c] = new f())._set(a), A.name = z[c], A) : null;
  }

  function m(c) {
    var M, a, A, o;

    for ("string" == typeof c && (c = [c]), M = 0; M < c.length; M++) A = B((a = c[M].split("|"))[0]), o = B(a[1]), n[A] = o, z[A] = a[0], n[o] = A, z[o] = a[1];
  }

  function T(c) {
    X(c.zones), m(c.links), function (c) {
      var M, a, A, o;
      if (c && c.length) for (M = 0; M < c.length; M++) a = (o = c[M].split("|"))[0].toUpperCase(), A = o[1].split(" "), i[a] = new N(a, A);
    }(c.countries), C.dataVersion = c.version;
  }

  function E(c) {
    return E.didShowError || (E.didShowError = !0, S("moment.tz.zoneExists('" + c + "') has been deprecated in favor of !moment.tz.zone('" + c + "')")), !!s(c);
  }
  ...
  function C(c) {
    var M = Array.prototype.slice.call(arguments, 0, -1),
        ...
        A = s(a),
        o = b.utc.apply(null, M);
    return A && !b.isMoment(c) && R(o) && o.add(A.parse(o), "minutes"), o.tz(a), o;
  }

  (a < 2 || 2 == a && A < 6) && S("Moment Timezone requires Moment.js >= 2.6.0. You are using Moment.js " + b.version + ". See momentjs.com"), f.prototype = {
    _set: function (c) {
      this.name = c.name, this.abbrs = c.abbrs, this.untils = c.untils, this.offsets = c.offsets, this.population = c.population;
    },
    _index: function (c) {
      for (var M = +c, a = this.untils, A = 0; A < a.length; A++) if (M < a[A]) return A;
    },
    countries: function () {
      var M = this.name;
      return Object.keys(i).filter(function (c) {
        return -1 !== i[c].zones.indexOf(M);
      });
    },
    parse: function (c) {
      for (var M, a, A, o = +c, b = this.offsets, p = this.untils, n = p.length - 1, i = 0; i < n; i++) if (M = b[i], a = b[i + 1], A = b[i ? i - 1 : i], M < a && C.moveAmbiguousForward ? M = a : A < M && C.moveInvalidForward && (M = A), o < p[i] - 6e4 * M) return b[i];
  ...
  }, C.version = "0.5.33", C.dataVersion = "", C._zones = p, C._links = n, C._names = z, C._countries = i, C.add = X, C.link = m, C.load = T, C.zone = s, C.zoneExists = E, C.guess = function (c) {
    return M && !c || (M = W()), M;
  }, C.names = function () {
    var c,
        M = [];

    for (c in z) z.hasOwnProperty(c) && (p[c] || p[n[c]]) && z[c] && M.push(z[c]);

    return M.sort();
  }, C.Zone = f, C.unpack = d, C.unpackBase60 = o, C.needsOffset = R, C.moveInvalidForward = !0, C.moveAmbiguousForward = !1, C.countries = function () {
    return Object.keys(i);
  }, C.zonesForCountry = function (c, M) {
    var a;
    if (a = (a = c).toUpperCase(), !(c = i[a] || null)) return null;
    var A = c.zones.sort();
  ...
  b.tz = C, b.defaultZone = null, b.updateOffset = function (c, M) {
    var a,
        A,
        o = b.defaultZone;
    void 0 === c._z && (o && R(c) && !c._isUTC && (c._d = b.utc(c._a)._d, c.utc().add(o.parse(c), "minutes")), c._z = o), c._z && (A = c._z.utcOffset(c), Math.abs(A) < 16 && (A /= 60), void 0 !== c.utcOffset ? (a = c._z, c.utcOffset(-A, M), c._z = a) : c.zone(A, M));
  }, g.tz = function (c, M) {
    if (c) {
      if ("string" != typeof c) throw new Error("Time zone name must be a string, got " + c + " [" + typeof c + "]");
      return this._z = s(c), this._z ? b.updateOffset(this, M) : S("Moment Timezone has no data for " + c + ". See http://momentjs.com/timezone/docs/#/data-loading/."), this;
    }

    if (this._z) return this._z.name;
  }, g.zoneName = P(g.zoneName), g.zoneAbbr = P(g.zoneAbbr), g.utc = D(g.utc), g.local = D(g.local), g.utcOffset = (h = g.utcOffset, function () {
    return 0 < arguments.length && (this._z = null), h.apply(this, arguments);
  }), b.tz.setDefault = function (c) {
    return (a < 2 || 2 == a && A < 9) && S("Moment Timezone setDefault() requires Moment.js >= 2.9.0. You are using Moment.js " + b.version + "."), b.defaultZone = c ? s(c) : null, b;
  };
  var _ = b.momentProperties;
