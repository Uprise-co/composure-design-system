!function (c, M) {
  ...
  function q(c, M) {
    ...
    for (A = 0; A < M.length; A++) o[A] = c[M[A]];
  ...
  function l() {
    ...
    var A,
      ...
      var c,
          ...
          z = new W(new Date(o, 0, 1)),
          b = [z];

      for (A = 1; A < 48; A++) (M = new W(new Date(o, A, 1))).offset !== z.offset && (c = X(z, M), b.push(c), b.push(new W(new Date(c.at + 6e4)))), z = M;

      for (A = 0; A < 4; A++) b.push(new W(new Date(o + A, 0, 1))), b.push(new W(new Date(o + A, 6, 1)));
      ...
      for (M = 0; M < z; M++) for (A in o = e[c[M].offset] || {}) o.hasOwnProperty(A) && (b[A] = !0);

      for (M in b) b.hasOwnProperty(M) && a.push(i[M]);
    ...
    for (o = 0; o < p.length; o++) {
      for (A = new f(s(p[o]), a), z = 0; z < a; z++) A.scoreOffsetAt(b[z]);

      n.push(A);
    }

    return n.sort(B), 0 < n.length ? n[0].zone.name : void 0;
  }
  ...
  function t(c) {
    ...
    for ("string" == typeof c && (c = [c]), M = 0; M < c.length; M++) z = T(A = (o = c[M].split("|"))[0]), a[z] = c[M], i[z] = A, u(z, o[2].split(" "));
  }
  ...
  function s(c, M) {
    ...
    var A,
        o = a[c];
    return o instanceof N ? o : "string" == typeof o ? (o = new N(o), a[c] = o) : p[c] && M !== s && (A = s(p[c], s)) ? ((o = a[c] = new N())._set(A), o.name = i[c], o) : null;
  }

  function E(c) {
    var M, A, o, z;

    for ("string" == typeof c && (c = [c]), M = 0; M < c.length; M++) o = T((A = c[M].split("|"))[0]), z = T(A[1]), p[o] = z, i[o] = A[0], p[z] = o, i[z] = A[1];
  }

  function m(c) {
    t(c.zones), E(c.links), function (c) {
      var M, A, o, z;
      if (c && c.length) for (M = 0; M < c.length; M++) A = (z = c[M].split("|"))[0].toUpperCase(), o = z[1].split(" "), n[A] = new d(A, o);
  ...
  function R(c) {
    var M = Array.prototype.slice.call(arguments, 0, -1),
        ...
        o = s(A),
        z = b.utc.apply(null, M);
    return o && !b.isMoment(c) && C(z) && z.add(o.parse(z), "minutes"), z.tz(A), z;
  }

  (A < 2 || 2 == A && o < 6) && S("Moment Timezone requires Moment.js >= 2.6.0. You are using Moment.js " + b.version + ". See momentjs.com"), N.prototype = {
    _set: function (c) {
    ...
    _index: function (c) {
      ...
      for (M = 0; M < o.length; M++) if (A < o[M]) return M;
    },
    countries: function () {
      var M = this.name;
    ...
    parse: function (c) {
      ...
      for (z = 0; z < n; z++) if (M = a[z], A = a[z + 1], o = a[z ? z - 1 : z], M < A && R.moveAmbiguousForward ? M = A : o < M && R.moveInvalidForward && (M = o), b < p[z] - 6e4 * M) return a[z];
  ...
  }, R.version = "0.5.31", R.dataVersion = "", R._zones = a, R._links = p, R._names = i, R._countries = n, R.add = t, R.link = E, R.load = m, R.zone = s, R.zoneExists = function c(M) {
    return c.didShowError || (c.didShowError = !0, S("moment.tz.zoneExists('" + M + "') has been deprecated in favor of !moment.tz.zone('" + M + "')")), !!s(M);
  }, R.guess = function (c) {
    return M && !c || (M = l()), M;
  }, R.names = function () {
    var c,
    ...
    return M.sort();
  }, R.Zone = N, R.unpack = r, R.unpackBase60 = z, R.needsOffset = C, R.moveInvalidForward = !0, R.moveAmbiguousForward = !1, R.countries = function () {
  ...
  b.tz = R, b.defaultZone = null, b.updateOffset = function (c, M) {
    var A,
        o = b.defaultZone;
    if (void 0 === c._z && (o && C(c) && !c._isUTC && (c._d = b.utc(c._a)._d, c.utc().add(o.parse(c), "minutes")), c._z = o), c._z) if (A = c._z.utcOffset(c), Math.abs(A) < 16 && (A /= 60), void 0 !== c.utcOffset) {
      var z = c._z;
      c.utcOffset(-A, M), c._z = z;
    } else c.zone(A, M);
  }, P.tz = function (c, M) {
    if (c) {
  ...
  }, P.zoneName = D(P.zoneName), P.zoneAbbr = D(P.zoneAbbr), P.utc = h(P.utc), P.local = h(P.local), P.utcOffset = (g = P.utcOffset, function () {
    return 0 < arguments.length && (this._z = null), g.apply(this, arguments);
  }), b.tz.setDefault = function (c) {
    return (A < 2 || 2 == A && o < 9) && S("Moment Timezone setDefault() requires Moment.js >= 2.9.0. You are using Moment.js " + b.version + "."), b.defaultZone = c ? s(c) : null, b;
  };
