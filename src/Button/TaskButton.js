...
const TaskStyle = Styled.div`
	padding: ${props =>
    props.last ? spacing.s5 : `${spacing.s5} ${spacing.s5} 0 ${spacing.s5}`};
      width: 100%;
      height: 100%;
	...
	@media (max-width: 1024px) {
		padding: ${props =>
      props.last ? spacing.s3 : `${spacing.s3} ${spacing.s3} 0 ${spacing.s3}`};
        ...
        <TaskLeftStyle>
          ...
          <TaskTextStyle>
            ...
            <Tiny className={`${last ? "m-b-0 m-t-0 " : "m-b-3 m-t-0 "}`}>
              {type === "bonus" || type === "homework" || type === "lessons"
                ? moment(task.time, "HH:mm:ss").format("mm:ss") + " min"
                : ""}
            </Tiny>
          </TaskTextStyle>
        </TaskLeftStyle>
